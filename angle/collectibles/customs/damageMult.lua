function isPlayerInSuccubusAura(player)
    -- 서큐버스 오라 반경(Rep 기준 약 90)
    local SUCCUBUS_RADIUS = 90
    local succubi = Isaac.FindByType(EntityType.ENTITY_FAMILIAR, FamiliarVariant.SUCCUBUS)
    for _, entity in ipairs(succubi) do
        if entity.Position:Distance(player.Position) < SUCCUBUS_RADIUS then
            return true
        end
    end
    return false
...
function getIsaacDamageMultiplier(player)
    local mult = 1
...
return getIsaacDamageMultiplier